...
export const hasRoles =
  (roles: Array<KeyOfMap<InternalRoleMap>>) => async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (
      Object.keys(endpointPermissions).every(permission =>
        endpointPermissions[permission as keyof Permissions] ? userPermissions[permission as keyof Permissions] : true,
      )
    ) {
      next();
    } else {
